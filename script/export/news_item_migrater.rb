...
class Migrator
	...
	def migrate(mod, l_post)
	  begin
	  	if ForagerCommon::NewsItem.find_by_original_url(l_post.original_url)
	  		mod.update(l_post.id, :is_migrated => 'd')
	  		print 'dup '
	  		return
	  	end
	  	
	  	if l_post.content_str.nil?
	  	  mod.update(l_post.id, :is_migrated => 'blank')
	  		print 'blank '
	  		return
	    end
			...
			c_post.original_url     = l_post.original_url
			c_post.meta_keywords    = "#{l_post.title} -- 成都保险咨询网: #{l_post.category}, 成都，保险, www.baoxian028.com"
			c_post.meta_description = "#{l_post.category}: #{l_post.title}, 成都，保险，商业保险，成都保险咨询网(www.baoxian028.com)提供专业的商业保险理财咨询服务!"
      c_post.body  = l_post.content_str
